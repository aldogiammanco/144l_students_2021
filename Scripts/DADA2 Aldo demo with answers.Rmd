---
title: "DADA2 aldo demo with answers"
...
A1: We see primers because our reads were long enough to the point that the machinery sequenced some of the primer on the end. We will use the MergePairs function and add trimOverhang=T to remove them later.
...
A2: We will trim our forward reads at base position 240, which will result in trimming the last 10 nucleotides. This is because this is where the quality begins to decline due to errors at the end of the run.
...
A3: The quality of the reverse reads is much poorer than the forward reads. Yes, the quality of these reads is still acceptable because DADA2 includes quality when running its error model, which allows the algorithm to remain strong will low quality reads.

Q4: Where will we trim our reverse reads and why?
A4: We will trim our reverse reads at position 150 because this is the position where the quality begins to drop off.
...
A5: If a base call is an N, then the sequencer ran into a position in which it did not know whether to put an A,T,C, or G, so it put a N. No, we will not use sequences that contain N's. maxN = 0 removes any reads with ambiguous base calls (N's).
...
A6: By using Excel, the average percentage of our reads that we retain was 93%.
...
A7: The relationship between quality score and error rate is that the error rate decreases when the quality score increases.
...
A9: The nmatch column tells us the number of matches in overlap for the forward and reverse reads - it tells us how many bases matched.
