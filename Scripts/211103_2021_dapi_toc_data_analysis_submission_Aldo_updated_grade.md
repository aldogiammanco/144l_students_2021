...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
...
A1: The control grows the slowest and maxes out at the lowest number of
cells. Kelp exudate grows the next slowest and maxes out at the next
lowest number of cells. Kelp + NP grows the next slowest and maxes out
at the next lowest number of cells. Glucose grows the fastest and maxes
out at the greatest number of cells. This does make sense with regard to
the oxygen drawdown data because glucose is using the most amount of
oxygen, so it would make sense that they are growing the fastest and are
most abundant. Also, the control uses the least amount of oxygen, so it
would make sense that they grow the slowest and are the least abundant.
The two kelp treatments use intermediate amounts of oxygen, so it makes
sense that their growth rates and abundances are in the middle as well.
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-4-1.png)<!-- -->
...
A2: For almost every treatment, there is a big decrease in biovolume
after T4 - the control only experiences a minor decrease. Nonetheless,
this is in contrast to the cell abundance of each treatment because
after T4, each treatment’s cell abundance continues to increase. Also,
kelp + NP reaches the greatest biovolume, whereas glucose reaches the
greatest cell abundance. This means that after T4, although cell
abundances continue to increase in each treatment, the cells are
becoming smaller in each treatment.
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-6-1.png)<!-- -->
...
A3: The limitation of using the of the DAPI data is that we only have 3
timepoints, thus our timepoints are very spaced out and we don’t have a
very clear understanding of the growth patterns that occurred between
the timepoints. We pretty much have data before/at the beginning of
exponential growth and data at the end of exponential growth, but
nothing really in between, so it is difficult to know what occurred in
between. With FCM, we took a measurement at every timepoint, therefore
we have a better representation of the growth patterns and can have a
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->
...
A4: The overall trends remain fairly similar up to T4, however after T4,
the bacterial carbon sharply begins to decrease when the biovolume is
taken into account with treatments kelp, kelp + NP, and glucose. Without
the biovolume, after T4, bacterial carbon either only decreases slightly
or continues to increase in all treatments. Also, although the general
trends remain similar up to T4, when you take biovolume into account,
the maximum amount of bacterial carbon increases in every treatment if
you compare the x-axes of the two graphs.
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->
...
A5: Yes, the patterns seen in all of these bar graphs line up with our
previous observations of cell abundance and oxygen drawdown. For
glucose, it grew the fastest, had the highest abundance, and biggest
oxygen drawdown. These are all reflected by glucose having the biggest
∆Cells, biggest growth rate, and shortest doubling time. For the
control, it grew the slowest, had the lowest abundance, and smallest
oxygen drawdown. These are reflected by the control hacing the smallest
∆Cells, smallest growth rate, and longest doubling time. Both kelp and
kelp + NP were intermediate in all these areas, with kelp + NP growing
faster and being more abundant, which is reflected by their intermediate
∆Cells, growth rates, and doubling times. Although kelp had more oxygen
drawdown than kelp + NP, it does not appear to be statistically
significant.
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-18-1.png)<!-- -->
...
A6: a) Our target was to add 10 umol of C relative to the control. b) We
did not hit our target in each treatment. It only appears we were
successful in the glucose treatment. c) We are not able to assess the
effect of the type of DOC added as well as we would have liked due to
the fact that the treatments did not all start with the same amount of
DOC. Thus, the amount of DOC added was a variable we introduced, so we
can moreso assess the effect of the amount of DOC added as opposed to
the type. d) TOC decreases over time in each treatment. Yes, this aligns
with our other data. The bacterial communities are growing in each
treatment, thus they must be taking up DOC from the environment and
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-20-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-21-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-22-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-23-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-24-1.png)<!-- -->
...
![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-25-1.png)<!-- -->

``` r
deldoc / deldoc_bv / bioav / bioav_bv / bge / bge_bv + plot_annotation(tag_levels = "a") + plot_layout(ncol = 2)
```

![](211103_2021_dapi_toc_data_analysis_submission_Aldo_updated_grade_files/figure-gfm/unnamed-chunk-26-1.png)<!-- -->
...
A7: a v b - Incorporating biovolume does not change our interpretation
of ∆DOC very much. The overall trend remains about the same and the
changes in the amounts between the two graphs don’t appear to be
statistically significant. c v d - This is similar to a v b in that
incorportating biovolume does not really change our interpretations. The
trends remain about the same as well as the values. e v f -
